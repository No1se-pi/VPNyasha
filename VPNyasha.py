...
USERNAME_REQUIRED_TEXT = (
    "Чтобы пользоваться ботом, нужен Telegram username.\n\n"
    "Добавь его в Telegram: Настройки → Изменить профиль → Имя пользователя. "
    "После этого вернись и нажми /start ещё раз."
)


def has_username(event_user) -> bool:
    return bool(event_user and event_user.username)


async def require_username_message(message: Message) -> bool:
    if is_admin(message.from_user.id) or has_username(message.from_user):
        return True
    await message.answer(USERNAME_REQUIRED_TEXT)
    return False


async def require_username_callback(callback: CallbackQuery) -> bool:
    if is_admin(callback.from_user.id) or has_username(callback.from_user):
        return True
    await callback.answer("Для использования бота нужен Telegram username.", show_alert=True)
    await callback.message.answer(USERNAME_REQUIRED_TEXT)
    return False
...
def main_menu_kb(user_id: Optional[int] = None) -> ReplyKeyboardMarkup:
    keyboard = [
        [KeyboardButton(text="Регистрация 📝"), KeyboardButton(text="Подключиться к VPN ⚡")],
...
def access_choice_kb_user() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="VPN", callback_data="buy_svc_vpn"),
                InlineKeyboardButton(text="Proxy", callback_data="buy_svc_proxy"),
            ],
            [InlineKeyboardButton(text="VPN + Proxy", callback_data="buy_svc_both")],
        ]
    )


def duration_choice_kb_user(service_code: str) -> InlineKeyboardMarkup:
    keyboard = []
    for duration_code, option in DURATION_OPTIONS.items():
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    text=f"{option['title']} — {price}₽",
                    callback_data=f"buy_dur_{service_code}_{duration_code}",
                )
            ]
        )
    keyboard.append([InlineKeyboardButton(text="Назад", callback_data="buy_back_services")])
...
def admin_service_kb(user_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="VPN", callback_data=f"adm_svc_{user_id}_vpn"),
                InlineKeyboardButton(text="Proxy", callback_data=f"adm_svc_{user_id}_proxy"),
            ],
            [InlineKeyboardButton(text="VPN + Proxy", callback_data=f"adm_svc_{user_id}_both")],
            [InlineKeyboardButton(text="Назад", callback_data=f"adm_user_{user_id}")],
        ]
    )


def admin_duration_kb(user_id: int, service_code: str) -> InlineKeyboardMarkup:
    keyboard = []
    for duration_code, option in DURATION_OPTIONS.items():
        keyboard.append(
            [
                InlineKeyboardButton(
                    text=option["title"],
                    callback_data=f"adm_dur_{user_id}_{service_code}_{duration_code}",
                )
            ]
        )
    keyboard.append([InlineKeyboardButton(text="Назад", callback_data=f"adm_grant_{user_id}")])
    return InlineKeyboardMarkup(inline_keyboard=keyboard)
...
def help_text() -> str:
    return (
        ...
        "<b>Как оплатить?</b>\n"
        "Нажми «Подключиться к VPN ⚡», выбери услугу и срок, оплати по ссылке и пришли чек администратору.\n\n"
        "<b>Как получить VPN по шагам?</b>\n"
        "1. Скачай AmneziaVPN.\n"
        "2. Оплати доступ.\n"
        "3. Открой бота и выбери тариф.\n"
        "4. Получи ключ.\n"
        ...
        "<b>Зачем нужен username?</b>\n"
        "Так админ видит, кому выдавать доступ. Добавь username: Настройки → Изменить профиль → Имя пользователя.\n\n"
        "<b>Как продлить?</b>\n"
        "Открой «Мой профиль 😎» → «Продлить доступ». При продлении VPN ключ остаётся тем же.\n\n"
...
def build_access_message(
    ...
    lines.extend(
        [
            f"Оплата продления: <a href='{h(settings.payment_url)}'>перейти к оплате</a>",
            "Готово! ⚡",
        ]
    )
    return "\n".join(lines)
...
async def grant_and_notify(
    ...
    service = SERVICE_OPTIONS[service_code]
    duration = DURATION_OPTIONS[duration_code]
    plan_label = f"{service['title']}, {duration['title']}"
    ...
    await safe_send_message(
        bot,
        user.user_id,
        build_access_message(user, service_code, duration_code, vpn_key, extension_only),
...
async def cmd_help(message: Message):
    prefix = "" if has_username(message.from_user) else f"{USERNAME_REQUIRED_TEXT}\n\n"
    await message.answer(prefix + help_text(), disable_web_page_preview=True)
...
async def handle_admin_user_lookup(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"Выберите, что выдавать {format_user_name(target_user)}:",
        reply_markup=admin_service_kb(target_user.user_id),
    )
...
async def handle_referral_code(message: Message, state: FSMContext):
    if not await require_username_message(message):
        await state.clear()
        return
    if not message.text:
...
@dp.message(RegistrationStates.waiting_nickname)
async def handle_nickname(message: Message, state: FSMContext):
    if not await require_username_message(message):
        await state.clear()
        return
    if not message.text:
        await message.answer("Отправь ник текстом или /cancel.")
        return
    nickname = message.text.strip()
    user = get_or_create_user(message.from_user.id, message.from_user.username)
    set_nickname(user.user_id, nickname)
    await state.clear()
    await message.answer(f"Ник обновлён: {h(nickname)}", reply_markup=main_menu_kb(message.from_user.id))
...
async def btn_registration(message: Message, state: FSMContext):
    ...
    await message.answer("Отправь ник:")
...
async def btn_connect(message: Message):
    ...
    await message.answer_sticker(sticker_id)
    await message.answer("Выбери, что подключить:", reply_markup=access_choice_kb_user())
...
async def callback_profile_back(callback: CallbackQuery):
    if not await require_username_callback(callback):
        return
...
async def callback_extend_access(callback: CallbackQuery):
    if not await require_username_callback(callback):
        return
    await callback.message.edit_text("Выбери, что продлить:", reply_markup=access_choice_kb_user())
    await callback.answer()


@dp.callback_query(F.data == "buy_back_services")
async def callback_buy_back_services(callback: CallbackQuery):
    if not await require_username_callback(callback):
        return
    await callback.message.edit_text("Выбери, что подключить:", reply_markup=access_choice_kb_user())
    await callback.answer()
...
async def callback_buy_service(callback: CallbackQuery):
    if not await require_username_callback(callback):
        return
    service_code = callback.data.removeprefix("buy_svc_")
    if service_code not in SERVICE_OPTIONS:
        await callback.answer("Неизвестная услуга", show_alert=True)
        return
    service = SERVICE_OPTIONS[service_code]
    await callback.message.edit_text(
        f"Выбрано: <b>{h(service['title'])}</b>\nТеперь выбери срок:",
        reply_markup=duration_choice_kb_user(service_code),
    )
    await callback.answer()
...
async def callback_buy_duration(callback: CallbackQuery):
    ...
    if len(parts) != 4:
        ...
        return
    _, _, service_code, duration_code = parts
    if service_code not in SERVICE_OPTIONS or duration_code not in DURATION_OPTIONS:
        await callback.answer("Ошибка выбора", show_alert=True)
        return
    ...
    await safe_send_message(
        ...
        (
            ...
            f"Срок: <b>{h(duration['title'])}</b>\n"
            f"Сумма: <b>{price}₽</b>\n"
            f"Рефкод пользователя: <code>{h(user.referral_code)}</code>\n\n"
            f"Выдать после оплаты: <code>/add_user {user.user_id}</code>"
        ),
        parse_mode=ParseMode.HTML,
    )
    ...
    await callback.message.edit_text(
        (
            ...
            f"Оплата: <a href='{h(settings.payment_url)}'>перейти к оплате</a>\n"
            f"После оплаты пришли чек @{h(settings.support_username)}."
        ),
        disable_web_page_preview=True,
    )
    await callback.answer()
...
async def callback_admin_grant(callback: CallbackQuery):
    ...
    if not user:
        ...
        return
    await callback.message.edit_text(
        f"Что выдать {format_user_name(user)}?",
        reply_markup=admin_service_kb(user.user_id),
    )
    await callback.answer()
...
async def callback_admin_service(callback: CallbackQuery):
    if not await ensure_admin_callback(callback):
        return
    _, _, user_id_raw, service_code = callback.data.split("_", 3)
    target_user_id = int(user_id_raw)
    user = users.get(target_user_id)
    if not user or service_code not in SERVICE_OPTIONS:
        await callback.answer("Ошибка выбора", show_alert=True)
        return
    await callback.message.edit_text(
        f"{format_user_name(user)} · {h(SERVICE_OPTIONS[service_code]['title'])}\nНа какой срок?",
        reply_markup=admin_duration_kb(target_user_id, service_code),
    )
    await callback.answer()
...
async def callback_admin_duration(callback: CallbackQuery, state: FSMContext):
    if not await ensure_admin_callback(callback):
        return
    _, _, user_id_raw, service_code, duration_code = callback.data.split("_", 4)
    target_user_id = int(user_id_raw)
    user = users.get(target_user_id)
    if not user or service_code not in SERVICE_OPTIONS or duration_code not in DURATION_OPTIONS:
        await callback.answer("Ошибка выбора", show_alert=True)
        return

    service = SERVICE_OPTIONS[service_code]
    duration = DURATION_OPTIONS[duration_code]
    if service["vpn"] and not user.vpn_key:
    ...
    granted_user, referral_bonus = await grant_and_notify(
        ...
        service_code,
        duration_code,
        extension_only=bool(service["vpn"] and user.vpn_key),
    )
    if not granted_user:
        await callback.answer("Пользователь не найден", show_alert=True)
        return
    key_text = "\nVPN ключ оставлен прежним." if service["vpn"] and user.vpn_key else ""
    bonus_text = "\nРеферальный бонус начислен 🎁" if referral_bonus else ""
    await callback.message.edit_text(
            ...
            if grace_until <= now:
                ...
                await safe_send_message(
                    ...
                    (
                        ...
                        f"Оплата восстановления: <a href='{h(settings.payment_url)}'>перейти к оплате</a>"
                    ),
                    parse_mode=ParseMode.HTML,
                    disable_web_page_preview=True,
                )
                key_text = f"\nКлюч: <code>{h(disabled_user.vpn_key)}</code>" if disabled_user.vpn_key else ""
                await safe_send_message(
                    bot,
                    settings.log_chat_id,
                    (
